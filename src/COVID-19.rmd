...
country = ifelse(exists("country"),country,"Egypt") 
...
# countryname for use in the text
...
I_max_d = round(doubleSigmoidalModel$endDeclinePoint_y, digits=0)
...
I_max_dsm = ifelse( ( a_d > 0 ) && ( I_final_d < 0.5 * I_max_d ),  
                    dsm_cum[ t_max ] +  2 * ( dsm_cum[t_d] - dsm_cum[ t_max ]) , -1 ) # not really fixed asymptote, -1 if there is no recovery yet
# Assess model
if(t_d <= t_max) { assessment = FALSE}
if(t_d >= nrow(DE) ) { assessment = FALSE }
```
...
Yesterday `r DE$Confirmed[nrow(DE)]` COVID-19 cases were known in `r countryname` [1]. Figure \@ref(fig:nowcast) illustrates how past Gompertz models (blue line) have predicted the last known number of cumulative COVID-19 cases (dashed black line). `r ifelse(convergent>0, paste0("The lightblue shaded area indicates that the Gompertz model of ",DE$Date[convergent]," (day ",convergent,") and all later models could predict the case number for ", yesterday, " with at most  ",criteria," percent difference to the actual case numbers. Hence, the Gompertz models could reliably predict the actual case numbers for the coming ", (nrow(DE)-convergent), " days."), "Unfortunately the Gompertz models did not yet converge and cannot provide reliable nowcasts and **we must regard all forecasts with great scepticism.**")`
