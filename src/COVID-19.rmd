...
country = ifelse(exists("country"),country,"Germany")
```
...
According to the Johns Hopkins University global COVID-19 data set [1] a total of `r last_confirmed` COVID-19 cases were reported in `r country` on `r yesterday`. This number of cases increased by `r round(last_increase_percent,digits=1)` percent over the cumulative total cases known one day earlier. At this rate of cases double every `r round(double_days, digits=0)` days and the basic reproduction rate $R_0$ in `r country` can be estimated at `r round(last_R0,digits=1)`, an estimate of the average number of other people infected by a single infected person. Figure \@ref(fig:daily-plot) shows the time line of new cases reported in the country (black points) together with a five day moving average (purple line) that smoothes daily fluctuations in the reporting that are due to fewer capacities on weekends, holidays and other reporting mishaps.
...
plot(DE$Date,p_dsm, type="l", col="orange", lwd=2, ylab="Double Sigmoidal model", xlab="Date")
# plot Week Lines
for (date in DE$Date) {
  if( as.numeric(strftime(as.Date(date, origin="1970-01-01"), "%u")) == 7 )
...
I_max_dsm = dsm_cum[ t_max ] + 2 * ( dsm_cum[t_d] - dsm_cum[ t_max ] ) # not really fixed asymptote
I_final_d = round(doubleSigmoidalModel$endDeclinePoint_y, digits=0)

```

## Week to week comparison of DSM models
...
The cumulative total of COVID-19 cases in `r country` is the number of all COVID-19 cases known up to yesterday and **includes all patients already recovered from the infection**. This corresponds to the _Infected (I)_ number in the standard epidemiological SIR model. Figure \@ref(fig:cumulative-plot) shows the data points of the time series together with a Gompertz model for the cumulative cases (red line) as well as the cumulated double-sigmoidal model (orange line). The dashed horizontal lines show the expected maximum number of cases of the respective models.

`r ifelse(I_final_d>10,paste0(" _Note:_ The cumulated double-sigmoidal model does not have an asymptote, since $I_{final_{d}}$ is not zero, the line can be considered as the number of infections when the infective wave ends and the endemic state starts. According to the DSM this happens when a total ", round(I_max_dsm, digits=0), " cases are reached."),"" )`
...
#     main=paste0("COVID-19 cases in ", country),
     ...
     ylab="Total Cases",
     xlab="Date",
     ylim=c(0,ifelse(I_max_c >= I_max_dsm, I_max_c, I_max_dsm)+100)
     )
# plot Week Lines
for (date in DE$Date) {
  if( as.numeric(strftime(as.Date(date, origin="1970-01-01"), "%u")) == 7 )
...
abline(h=I_max_dsm, lty=2, lwd=2, col="orange")
...
## Week to week comparison of Gompertz models
...
| :---------------------- | --------------------------------------------- | ------------------------------------------------- | :--------------------------------------------------------------------------- |
| $I_{max_{c}}$ Total number of cases | `r round(gomp$m$getAllPars()["a"], digits=0)`                  | `r round(gomp_ago$m$getAllPars()["a"], digits=0)`                  | `r round(gomp$m$getAllPars()["a"], digits=0)  - round(gomp_ago$m$getAllPars()["a"], digits=0)`                |
...
for(i in rev( seq(1,nrow(models2) ) ) ) {
    if (( comparison$gomp[i] >= (1 - criteria/100) * DE$Confirmed[nrow(DE)] ) 
...
Yesterday `r DE$Confirmed[nrow(DE)]` COVID-19 cases were known in `r country` [1]. Figure \@ref(fig:nowcast) illustrates how past Gompertz models (blue line) have predicted the last known number of cumulative COVID-19 cases (dashed black line). `r ifelse(convergent>0, paste0("The vertical dashed blue line indicates that since ",DE$Date[convergent]," (day ",convergent,") all models were at most  ",criteria," percent off the case numbers reported yesterday. Hence, the Gompertz model could predict the development for ", (nrow(DE)-convergent), " days ahead."), "Unfortunately the Gompertz models did not yet converge and cannot provide reliable nowcasts and we must regard all forecasts with great sceptisism.")`
...
d = data.frame(
  ...
  I_max_c=c( round(gomp$m$getAllPars()["a"], digits=0) ),
  alpha_c=c( gomp$m$getAllPars()["c"] ),
  beta_c=c(gomp$m$getAllPars()["b"]),
