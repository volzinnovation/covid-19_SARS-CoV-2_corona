...
options(scipen = 999)
# Use country from build script parameter, or default to German
country = ifelse(exists("country"),country,"France")
```

---
title: "Growth of reported Covid-19 cases in `r country`"
author: Raphael Volz (raphael.volz@hs-pforzheim.de)
date: DRAFT work in progress, last updated on `r Sys.Date()`
...
# Look only to data when more than 100 confirmed infections are there
DE = subset(DE, DE$Confirmed > 100)
# Last known cases
...
According to the Johns Hopkins University global COVID-19 data set [1] a total of `r last_confirmed` COVID-19 cases were reported in `r country` on `r yesterday`. This number of cases increased by `r round(last_increase_percent,digits=1)` percent over the cumulative total cases known one day earlier. At this rate of cases double every `r round(double_days, digits=0)` days and the basic reproduction rate $R_0$ in `r country` can be estimated at `r round(last_R0,digits=1)`, an estimate of the average number of other people infected by a single infected person. Figure \@ref(fig:daily-plot) shows the new cases reported since 100 cases are known in the country (black points) together with a five day moving average (purple line) that smoothes daily fluctuations in the reporting that are due to fewer capacities on weekends, holidays and other reporting mishaps.
...
The model can obtain predictions related to an observed maximum of daily new cases $I_{max_{d}}$ and an endemic number of daily cases $I_{final_{d}}$ as an asymptote (that is ideally 0). Let $f_{d}(t)$ be a piecewise function that switches between the growth and decay phase at $t_{max}$, such that
...
| $\alpha_{i}$ Slope of increase phase    | `r round(doubleSigmoidalModel$slope1Param_Estimate, digits=2)`                                                             | `r round(dsm_ago$slope1Param_Estimate, digits=2)`                                                             | `r round(doubleSigmoidalModel$slope1Param_Estimate - dsm_ago$slope1Param_Estimate, digits=2)` |
...
| $t_{d}$ Midpoint of decrease phase | `r DE$Date[round(doubleSigmoidalModel$midPoint2_x, digits=0)][1]` (Day `r round(doubleSigmoidalModel$midPoint2_x, digits=0)`) | `r DE$Date[round(dsm_ago$midPoint2_x, digits=0)][1]` (Day `r round(dsm_ago$midPoint2_x, digits=0)`)              | `r round(doubleSigmoidalModel$midPoint2_x, digits=0) -round(dsm_ago$midPoint2_x, digits=0)`   |

In order to address the shortcoming of model instability under new data, we can attempt to fit a model on cumulative cases instead.

# Cumulative cases

The cumulative total of COVID-19 cases in `r country` is the number of all COVID-19 cases known up to yesterday and **includes all patients already recovered from the infection**. This corresponds to the _Infected (I)_ number in the standard epidemiological SIR model. Figure \@ref(fig:cumulative-plot) shows the data points of the time series together with a Gompertz model for the cumulative cases (red line) as well as the cumulated double-sigmoidal model (orange line).

```{r cumulative-plot, fig.cap="Cumulative cases with fitted models", echo=FALSE}
...
# plot Gompertz model
lines(DE$Date, p_gomp, col="red", lwd=2)
# plot DSM model
lines(DE$Date, dsm_cum, col="orange", lwd=2)
...
This function is the integral of the probability density function (PDF) developed by Benjamin Gompertz in 1825 to model human mortality, hence his probability densitiy function (PDF) is the derivative of the CDF:

$$\frac{\partial f_{c}(t)}{\partial t} = I_{max_{c}} * \beta_{c} * \alpha_{c} * e^{ - \beta_{c} * e^{ - \alpha_{c} * t } - \alpha_{c} * t} $$
...
| :---------------------- | --------------------------------------------- | ------------------------------------------------- | :--------------------------------------------------------------------------- |
| $I_{max_{c}}$ Expected maximum | `r round(gomp$m$getAllPars()["a"], digits=0)`                  | `r round(gomp_ago$m$getAllPars()["a"], digits=0)`                  | `r round(gomp$m$getAllPars()["a"], digits=0)  - round(gomp_ago$m$getAllPars()["a"], digits=0)`                |
| $\alpha_{c}$ Growth rate      | `r gomp$m$getAllPars()["c"]`                  | `r gomp_ago$m$getAllPars()["c"]`                  | `r gomp$m$getAllPars()["c"] - gomp_ago$m$getAllPars()["c"]`                  |
| $\beta_{c}$ Time shift       | `r round(gomp$m$getAllPars()["b"], digits=0)` | `r round(gomp_ago$m$getAllPars()["b"], digits=0)` | `r round(gomp$m$getAllPars()["b"] - gomp_ago$m$getAllPars()["b"], digits=0)` |

## Parameter convergence of the Gompertz model

Figures \@ref(fig:evol-Imax), @ref(fig:evol-alpha), \@ref(fig:evol-beta) present how the model parameters change over time as new cases are reported.

```{r, echo=FALSE}
# find first date of infections (over 100 cases)
min_d = min(DE$Date)
...
Putting in future dates into the current model allows to project future cases. _Note:_ The actual development can differ from model predictions, for example if social distancing norms change or methods in data collection are altered. If such changes happen, a model fitted to the past is no longer valid.

## Nowcast

Yesterday `r DE$Confirmed[nrow(DE)]` COVID-19 cases were known in `r country` [1]. Figure \@ref(fig:nowcast) illustrates how past models (grey line) have predicted the last known number of cumulative COVID-19 cases (dashed black line).
...
Using the current model fitted to all available data we can estimate the future number of total COVID-19 cases in the next couple of days using either model or the ensembled mean prediction of both models.
...
Epidemiological models are useful for policy makers, e.g. to assess (lack of) ICU beds, required (additional) restrictions of daily life, etc. However models require (often disputed) assumptions for model parameters. The data-driven approach demonstrated here avoids parameter assumptions and uses past data to fit models that not only describe the past developments but provide reliable forecasts for the nearer future of an epidemic. Next to policy makers, the general public can use the projections to assess their expectations when business or schools might reopen (or reclose).

As a next step the convergence of models could be investigated more closely to assess whether model divergence is an early indicator for a general change in infective behavior. Additional data about the population and population structure could be used to compare between countries, for example to assess the effectiveness of different public policies for controlling the disease.
...
## Cumulative (Gompertz) model history

The following table states the known COVID-19 infections and duration of the epidemic in the country on a given day. It also states all _three parameters_ of the model estimated with data available on that date and reports the root mean square error (RMSE) of the model on the available data RMSE (Training). The information made available after model creation is used as a test of the model precision on unknown data and reported as RMSE (Test). The forecast for today of the given model is also provided together with the relative delta to the actual number reported.
